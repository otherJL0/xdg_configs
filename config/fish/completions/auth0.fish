...
function __auth0_perform_completion
    __auth0_debug "Starting __auth0_perform_completion"
    ...
    set -l requestComp "AUTH0_ACTIVE_HELP=0 $args[1] __complete $args[2..-1] $lastArg"
...
# - Obtain the completions and store them in the global __auth0_comp_results
# - Return false if file completion should be performed
function __auth0_prepare_completions
    __auth0_debug ""
    __auth0_debug "========= starting completion logic =========="

    # Start fresh
    set --erase __auth0_comp_results

    set -l results (__auth0_perform_completion)
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __auth0_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__auth0_comp_results)
        set --global __auth0_comp_results $completions
        __auth0_debug "Filtered completions are: $__auth0_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__auth0_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__auth0_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __auth0_comp_results $split[1] $split[1].
                __auth0_debug "Completions are now: $__auth0_comp_results"
            end
        end
...
# The call to __auth0_prepare_completions will setup __auth0_comp_results
